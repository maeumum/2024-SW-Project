class Car:
    ...
    def accelerate(self):
        """엔진이 켜져 있을 때만 속도를 증가시킴"""
        if not self.__engine_on:
            print("[ACCELERATE] Cannot accelerate: Engine is off.")
            return
        if self.__speed >= 150:  # 최대 속도 제한
            print("[ACCELERATE] Speed limit reached. Cannot exceed 150 km/h.")
            return
        self.__speed += 10  # 속도 증가
        print(f"[ACCELERATE] Speed increased to {self.__speed} km/h.")
        if self.gui:
