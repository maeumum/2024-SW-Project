...
def execute_command_callback(command, car_controller,gui):
    ...
    if car_controller.car.speed > 0:  # 주행 시, 문&트렁크 open, unlock 명령 무시
        ...
        if command == "TRUNK_OPEN": return  # 주행 시 트렁크 open 명령 무시
    ...
    if command == "ENGINE_BTN":
        ...
        if car_controller.car.left_door_status == "CLOSED" \
            ...
            car_controller.toggle_engine(gui)  # 시동 ON / OFF
...
if __name__ == "__main__":
    ...
    # GUI는 메인 스레드에서 실행
    gui = CarSimulatorGUI(car,car_controller, lambda command: execute_command_callback(command, car_controller,gui))
